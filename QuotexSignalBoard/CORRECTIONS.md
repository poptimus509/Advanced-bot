...
## 2026-09 correctness/diagnostic build additions

See `FIXED_BUILD_README.md` for the current changes. This build adds data-pipeline diagnostics, repairs ADX/RSI edge cases, prevents REST history from masquerading as a live quote, deduplicates candle epochs, rebuilds complete higher-timeframe bars from 1M history, and exposes `/api/history-diagnostics`.

`SIGNAL_THRESHOLD_CALL_PUT` intentionally remains at 6; strategy profitability has not been asserted or tuned by these correctness fixes.
