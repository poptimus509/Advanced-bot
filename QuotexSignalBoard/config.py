...
# Current strategy threshold. Kept unchanged in this correctness-focused build.
SIGNAL_THRESHOLD_CALL_PUT = 6

# Emit detailed history-pipeline diagnostics in Render logs.
...
# At the start of each minute, if the just-closed 1M candle has not been
# finalized by the live tick stream yet, fetch only a few recent candles for
# the missing symbols. Requests are sent in parallel so this stays inside the
# normal scan window.
