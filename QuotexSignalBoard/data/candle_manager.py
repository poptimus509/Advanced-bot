...
class CandleManager:
    ...
    def seed_historical_candles(self, timeframe: str, raw_candles: List[dict], current_server_epoch: Optional[int] = None):
        ...
        with self._mutex:
            existing_epochs = {c.epoch for c in self._closed_candles[timeframe]}
            ...
            if forming is None and sorted_raw:
                ...
                try:
                    last_epoch = int(last_raw.get("epoch", last_raw.get("time", 0)))
                    if last_epoch + tf_sec > now_epoch:
                        self._forming_candles[timeframe] = {
                            "epoch": last_epoch,
                            "open": float(last_raw.get("open", 0.0)),
                            "high": float(last_raw.get("high", 0.0)),
                            "low": float(last_raw.get("low", 0.0)),
                            "close": float(last_raw.get("close", 0.0)),
                            "ticks_count": last_raw.get("count", last_raw.get("ticks_count", 1)),
                        }
                        new_candles = [c for c in new_candles if c.epoch != last_epoch]
                except Exception:
                    pass
            ...
            self._closed_candles[timeframe] = deque(merged, maxlen=self.max_history)
