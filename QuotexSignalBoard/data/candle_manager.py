        ...
        with self._mutex:
            ...
            for raw in sorted_raw:
                try:
                    ...
                    # A candle that was forming during an earlier REST seed
                    # eventually becomes closed.  Do NOT keep skipping the
                    # same epoch forever: once the server clock has crossed
                    # its close boundary, the REST candle is authoritative
                    # and must replace/promote the stale forming snapshot.
                    ...
                    if forming_epoch is not None and epoch == forming_epoch:
                        self._forming_candles[timeframe] = None
                        forming_epoch = None
                except Exception:
                    continue

            # Cold start / REST catch-up: preserve the newest still-forming
            # bar after any older forming bar has just been promoted to
            # closed history.  This is what lets a process recover cleanly
            # even when the first live tick of the new minute is delayed.
