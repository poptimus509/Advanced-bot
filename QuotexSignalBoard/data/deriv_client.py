...
class DerivClient:
    ...
    def fetch_historical_candles_batch_sync(
        ...
        """
        Fetch several candle histories in parallel over the same WebSocket.

        The previous implementation waited for each symbol before sending the
        next request.  Around a minute boundary that could make a 16-pair
        refresh too slow for the scanner's entry window.  This version sends
        all requests first, then waits against one shared deadline.
        """
        ...
        for job in jobs:
            ...
            try:
                ...
                pending.append((key, target_sym, count, granularity, req_id, event))
            except Exception as exc:
                logger.error("Error requesting history for %s: %s", symbol, exc)
                results[key] = []
                self._pending_requests.pop(req_id, None)
                self._request_results.pop(req_id, None)
        ...
        for key, target_sym, count, granularity, req_id, event in pending:
            try:
                remaining = max(0.0, deadline - time.monotonic())
                if event.is_set() or (remaining > 0 and event.wait(timeout=remaining)):
                    candles = self._request_results.pop(req_id, [])
                    if isinstance(candles, list) and candles:
                        results[key] = candles
                    else:
                        results[key] = []
                        failed.append((key, target_sym, count, granularity))
                else:
                    results[key] = []
                    failed.append((key, target_sym, count, granularity))
            finally:
        ...
        if allow_fallback:
            for key, target_sym, count, granularity in failed:
                fallback = self._fetch_single_history(target_sym, count, granularity)
                if fallback:
                    results[key] = fallback

        for job in jobs:
            results.setdefault(job.get("key", ""), [])
        return results
